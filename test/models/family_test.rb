...
class FamilyTest < ActiveSupport::TestCase
  ...
  test 'fill_color 1 lit gizmos' do
    assert_equal "#0000ff", family(:gizmos => [{:status => true, :color => "b"}]).fill_color
  end

  test 'fill_color 1 unlit gizmos' do
    assert_equal "#dddddd", family(:gizmos => [{:status => false, :color => "r"}]).fill_color
  end

  test 'fill_color 2 lit gizmos' do
    assert_equal "#ff00ff", family(:gizmos => [{:status => true, :color => "r"},
                                               {:status => true, :color => "b"}]).fill_color
  end
